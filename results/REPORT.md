...
- [compression_rate_faceted.svg](results/report_flores60/faceted_plots/compression_rate_faceted.svg)
- [vocabulary_utilization_faceted.svg](results/report_flores60/faceted_plots/vocabulary_utilization_faceted.svg)

**core** (`results/report_core/faceted_plots`):
- [compression_rate_faceted.svg](results/report_core/faceted_plots/compression_rate_faceted.svg)
- [vocabulary_utilization_faceted.svg](results/report_core/faceted_plots/vocabulary_utilization_faceted.svg)
...
- **Models:** nanochat-based transformers; every comparison is made *within a single vocabulary size*, so transformer-matrix parameters and the token budget match across the pair. Token budget = 10.5 × (transformer matrices + lm_head) (nanochat's Kaplan version of the Chinchilla rules that counts embedding parameters; here, it ends up being the same as the standard 20x non-embedding paramters rule); µP for learning-rate transfer; fixed batch sizes; matrix LR 0.02 (chosen by a 5-point sweep).
